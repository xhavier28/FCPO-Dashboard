...
Predicts whether the current curve shape will still be the same shape
at 4-week and 12-week horizons (binary persistence question).
NOT a shape predictor — has no top-1 shape output.

Uses per-shape Logistic Regression with StandardScaler, trained on
2017-2024 weekly panel. Interpretable LR coefficients validate
Phase A driver hypotheses.

Wraps: research/02. phase_identification/phase_b_persistence_model.py
...
def predict_persistence(date, shape=None):
    """
    Predicts whether the current shape will still be the same shape
    at 4-week and 12-week horizons (binary persistence).
    NOT a shape predictor — has no top-1 shape output.
    Wraps: research/02. phase_identification/phase_b_persistence_model.py
